...
export const updateMatchData = async (req, res) => {
  const { matchId } = req.query;
  const { score, bowlCount, strikeId, bowler_id, nonStrikeId, newInnings } =
    req.body;
    ...
    switch (newInnings) {
      case false:
        ...
        if (score === "W" && matchData.currentBatsman.nextBatsman.index <= 10) {
          ...
          let newBatsman =
            players[matchData.currentBattingTeam.country][nextBatsmanIndex];
          matchData.currentBatsman.strike.id = newBatsman.id;
          matchData.currentBatsman.nextBatsman.index = nextBatsmanIndex + 1;
        }
        // change the strike
        if (
          (score !== "W" && score % 2 && bowlCount < 5) ||
          ((score % 2 == 0 || score === "W") && bowlCount === 5)
        ) {
          let strikeId = matchData.currentBatsman.strike.id;
          matchData.currentBatsman.strike.id =
            matchData.currentBatsman.nonStrike.id;
          matchData.currentBatsman.nonStrike.id = strikeId;
        }

        if (
          matchData.target &&
          matchData.currentBattingTeam.totalRuns >= matchData.target
        ) {
          matchData.matchResult = `${
            matchData.currentBattingTeam.country
          } has won the match by ${
            10 - matchData.currentBattingTeam.wicketsFallen
          } wickets`;
        } else if (
          matchData.target &&
          (matchData.currentBattingTeam.oversPlayed ===
            matchData.matchOver * 6 ||
            matchData.currentBattingTeam.wicketsFallen === 10)
        ) {
          if (matchData.target - 1 === matchData.currentBattingTeam.totalRuns) {
            matchData.matchResult = `Match Drawn`;
          } else {
            matchData.matchResult = `${
              matchData.currentBowlingTeam.country
            } has won the match by ${
              matchData.BattedFirstScores.totalRuns -
              matchData.currentBattingTeam.totalRuns
            } runs`;
          }
        }
      ...
      case true:
        ...
        // storing the scores of the firs batted team
        ...
        data.BattedFirstScores.oversPlayed =
          data.currentBattingTeam.oversPlayed;
        data.BattedFirstScores.wicketsFallen =
          data.currentBattingTeam.wicketsFallen;
        // ready for second innings score update
        ...
        data.currentBatsman.nonStrike.id =
          players[data.battingSecondTeam][1].id;
